...
class CheeseAdminIndexView(flask_admin.AdminIndexView):
    ...
    def sort_by_column(self, surveys, sort, reverse):
        ...
        if sort == 'survey':
            ...
            return sort_surveys(lambda x: x.ward.name.lower() if x.ward else 'None')
...
def submit_pre_survey_details():
    ...
    if request.method=='POST':
        if helpers.validate_form_on_submit(form):
            ...
            message = 'For '+details.householders_name+', '+details.address_line+', '+details.postcode  \
                        +'\n'+str(datetime.datetime.today())+': '+database_url
            mail.send(Message(subject=subject,
                              body=message,
                              recipients=current_app.config['WATCHERS']))
...
membership             = bp.route('/membership')(flatpage_template)
